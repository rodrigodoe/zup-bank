...
public class CustomizedExceptionHandlers {
	...
	public List<CustomizedValidationError> handleValidationExceptions(MethodArgumentNotValidException ex) {
		...
		ex.getBindingResult().getAllErrors().forEach((error) -> {
			CustomizedValidationError erro = new CustomizedValidationError(error.getDefaultMessage(), ((FieldError) error).getField());
			errors.add(erro);
		});
		return errors;
	}
	...
	public CustomizedValidationError handleDateTimeParseException(Exception ex) {
		CustomizedValidationError customError = new CustomizedValidationError("FORMATO DE DADA INVALIDA");
		return customError;
	}
